...
public class UserController {
    ...
    public ResponseEntity<Page<User>> getUserList(
            @Parameter(description = "页码", example = "1") @RequestParam("current") Integer current,
            @Parameter(description = "每页大小", example = "10") @RequestParam("size") Integer size){
        
        Page<User> page = new Page<>(current, size);
        Page<User> result = userService.getUserPage(page);
    ...
    public ResponseEntity<User> getUserById(
            @Parameter(description = "用户ID", example = "1") @RequestParam("id") Long id) {
    ...
    public ResponseEntity<User> updateUser(
            @Parameter(description = "用户ID", example = "1") @RequestParam("id") int id,
